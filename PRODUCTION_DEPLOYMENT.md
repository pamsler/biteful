...
scp docker-compose.production.yml user@server:/opt/biteful/
scp .env.production.example user@server:/opt/biteful/
```
...
# Volumes Backup
docker run --rm -v biteful_postgres_data:/data -v $(pwd):/backup alpine tar czf /backup/postgres_data_backup.tar.gz -C /data .
docker run --rm -v biteful_product_uploads:/data -v $(pwd):/backup alpine tar czf /backup/uploads_backup.tar.gz -C /data .
```
...
# Volumes Restore
docker run --rm -v biteful_postgres_data:/data -v $(pwd):/backup alpine sh -c "cd /data && tar xzf /backup/postgres_data_backup.tar.gz"
```
...
```bash
# DB Health Check
docker exec biteful-db pg_isready -U mealuser
...
Erstellen Sie `/etc/systemd/system/biteful.service`:
...
Aktivieren:
```bash
sudo systemctl enable biteful
sudo systemctl start biteful
sudo systemctl status biteful
```
